...
public abstract class BaseRepository<I> {
    ...
    protected <T> List<I> findBySecundaryKeyAndAttribute(final T pk, final String indexName, String attributeName, String attributeValue, String operator) {
        List<I> resultList = new ArrayList<>();
        DynamoDbIndex<I> secIndex = this.getIndex(indexName);

        QueryConditional queryConditional = QueryConditional
                .keyEqualTo(Key.builder().partitionValue((String) pk).build());

        Iterable<Page<I>> results = secIndex.query(QueryEnhancedRequest.builder()
                .queryConditional(queryConditional)
                .filterExpression(getExpression(attributeName, attributeValue, operator))
    ...
    public <T> List<I> queryFilterItems(final T pk){
        ...
        Iterable<Page<I>> results = table.query(QueryEnhancedRequest.builder()
                .queryConditional(queryConditional)
                .consistentRead(getConsistentRead())
                .build());
